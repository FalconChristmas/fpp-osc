...
# Nothing to do here in the normal case.  fppd loads libfpp-osc.so once at its own
...
if [ ! -f libfpp-osc.so ]; then
    echo "Running fpp-osc PreStart Script - libfpp-osc.so is missing, rebuilding"
    make "SRCDIR=${SRCDIR}"
fi
